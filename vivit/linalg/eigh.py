...
class gram_eigendecomposition:
    """Context manager for setting the Gram matrix eigendecomposition function."""

    EIGENDECOMPOSITION = lambda tensor: tensor.symeig(eigenvectors=True)  # noqa: E731

    def __init__(
        self, eigendecomposition: Callable[[Tensor], Tuple[Tensor, Tensor]]
    ) -> None:
        """Store function to be used for eigendecomposition inside the context.

        Args:
            eigendecomposition: Method used for eigendecomposition.
        """
        self._new_eigendecomposition = eigendecomposition

    def __enter__(self):
        """Set eigendecomposition method."""
        self._old_eigendecomposition = gram_eigendecomposition.EIGENDECOMPOSITION
        gram_eigendecomposition.EIGENDECOMPOSITION = self._new_eigendecomposition
    ...
    def __exit__(self, type, value, callback):
        ...
        # noqa: DAR101
        """
        gram_eigendecomposition.EIGENDECOMPOSITION = self._old_eigendecomposition
...
class EighComputation:
    ...
    def get_group_hook(
        ...
        def group_hook(
            ...
            eigendecomposition = gram_eigendecomposition.EIGENDECOMPOSITION
            gram_evals, gram_evecs = eigendecomposition(reshape_as_square(gram_mat))
